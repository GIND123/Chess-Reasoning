...
ORDER = ["base_model", "sft", "m3_move_only", "m6_composite", "m3v2", "m6v2", "a3v2"]
SHORT = {"base_model": "Base", "sft": "SFT", "m3_move_only": "M3", "m4_sparse": "M4",
         "a3_no_coverage": "A3", "m6_composite": "M6",
         "m3v2": "M3-v2", "m6v2": "M6-v2", "a3v2": "A3-v2"}
LONG = {"base_model": "Base", "sft": "SFT", "m3_move_only": "M3 move-only",
        "m4_sparse": "M4 sparse", "a3_no_coverage": "A3 no-coverage",
        "m6_composite": "M6 composite", "m3v2": "M3-v2 dense",
        "m6v2": "M6-v2 dense", "a3v2": "A3-v2 dense"}
# arms trained with the dense (unclipped) move reward
DENSE = {"m3v2", "m6v2", "a3v2"}
...
def fig_grounding(d, out):
    ...
    cols = [SERIES[1] if t in DENSE else SERIES[0] for t in ORDER]
    ...
    ax1.text(0.5, 0.868, "orange = dense move reward", fontsize=7.5, color=SERIES[1])
    ax1.text(0.5, 0.855, "M6-v2 > SFT  +6.8 pp,  p < 0.001", fontsize=7.5,
             color=STATUS["good"])

    labels, diffs, ps = [], [], []
    for name, v in d["comparisons"].items():
        if name.endswith("claim_precision") and all(
                x in LONG for x in name.split(" :: ")[0].split(" vs ")):
